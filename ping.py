...
# elegance of this is questionnable I agree
# this is for compat 2.7 and 3.4+
...
def calculate_checksum(source_string):
    ...
    while count < countTo:
        ...
        sum = sum + (_ord(hiByte) * 256 + _ord(loByte))
        count += 2
...
class Ping(object):
    def __init__(self, destination, timeout=1000, packet_size=55, own_id=None,verbose=True):
        ...
        try:
            ...
            if self.verbose : self.print_start()
    ...
    def send_one_ping(self, current_socket):
        ...
        data = _bytes(padBytes)
        if self.verbose : print(len(header),len(data),len(padBytes))
        ...
        try:
            ...
            if self.verbose : print("General failure (%s)" % (e.args[1]))
            current_socket.close()
            return
...
class IsUp(Ping) :
    def __init__(self, destination, timeout=1000, packet_size=55, own_id=None):
        super(IsUp,self).__init__(destination, timeout, packet_size, own_id,False)

    def isup(self):
        """
        quietly send and receive one ping, and return delay or False if it fails at some point
        """
        self.setup_signal_handler()
...
def isup_ping(hostname, timeout=1000, packet_size=55):
    p = IsUp(hostname, timeout, packet_size)
    print(p.isup())
...
if __name__ == '__main__':
    ...
    elif len(sys.argv) == 3 and sys.argv[2] == 'isup':
        isup_ping(sys.argv[1])
    else:
